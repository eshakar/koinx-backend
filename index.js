...
// Routes
import taxHarvestingRoutes from './routes/taxHarvestingRoutes.js';
import reconciliationRoutes from './routes/reconciliationRoutes.js';

// Ingestion and engine for seeding
...
app.use(taxHarvestingRoutes);
app.use(reconciliationRoutes);

// Seeding holdings and gains data
...
const seedDatabase = async () => {
  try {
    ...
    const gainsCount = await CapitalGain.countDocuments();
    if (gainsCount === 0) {
      console.log('Seeding initial capital gains...');
      await CapitalGain.create(initialCapitalGains);
    }

    // Auto-seed transactions from CSV
    const runsCount = await ReconciliationRun.countDocuments();
    if (runsCount === 0) {
      console.log('No reconciliation runs found in database. Auto-seeding transaction CSVs...');
      ...
      if (fs.existsSync(userCsvPath) && fs.existsSync(exchangeCsvPath)) {
        const userResult = await ingestTransactions(userCsvPath, 'user', defaultRunId);
        const exchangeResult = await ingestTransactions(exchangeCsvPath, 'exchange', defaultRunId);
        
        await runReconciliation(defaultRunId, {
          timestampToleranceSeconds: 300,
          quantityTolerancePct: 0.01
        });
        console.log('Transaction CSV auto-seeding completed.');
      } else {
        console.warn(`Transaction CSV files not found (searched: ${userCsvPath}). Skipping CSV auto-seed.`);
      }
    } else {
      console.log('Reconciliation runs already exist. Skipping CSV auto-seed.');
    }
...
app.listen(PORT, () => {
  ...
  console.log(`Database engine is: ${db.isMongo() ? 'MongoDB' : 'JSON local file'}`);
});
